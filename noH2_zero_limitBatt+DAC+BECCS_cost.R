...
df_all_xy <- cbind(df_all_x, df_all_y) # combine both vectors

colnames(df_all_xy) <- c('Abat Mt C','Price 1990$/t C') # rename
...
# manually check that the first row of every year starts with a zero (1st price point is always zero)
...
Region_192 <- rep(Region_32, each=6)

Region_1344 <- rep(Region_192, times=7)

# create price level vector
Price_level <- rep(c('0%', '20%', '40%', '60%', '80%', '100%'), times=224)


# bring all together into one dataframe
df_all_xy <- cbind(df_all_xy, Region_1344, Price_level) 
...
delta <- df_all_xy$'Abat Mt CO2' - dummy
...
Region_224 <- rep(Region_32, times=7)

periods_7 <- rep(c("2020",
                 "2025",
                 "2030",
                 "2035",
                 "2040",
...
periods_224 <- rep(periods_7, each=32)

mit_cost_country <- cbind(mit_cost_country, Region_224, periods_224)
...
discount_period <- as.numeric(periods_224) - t
...
# combine, rename columns, change order, and improve readability:
...
t <- rep(c(2020), each=7)
discount_period <- as.numeric(periods_7) - t
